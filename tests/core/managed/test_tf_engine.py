...
from pyshac.core.managed import tf_engine as engine
...
""" Tensorflow Evaluators """


def evaluation_simple_tf(session, worker_id, params):
    """
    ...
    Returns:
        float
    """
    values = tf.convert_to_tensor(list(params.values())[:3], dtype=tf.float32)
    metric = tf.reduce_sum(values)
    metric = session.run(metric)

    print('objective value =', metric)
    return metric


def evaluation_hartmann6_tf(session, worker_id, params):
    """
    ...
    xx = tf.convert_to_tensor(np.array([list(params.values())]).T, dtype=tf.float32)
    ...
    outer = session.run(outer)

    return -outer


def evaluation_branin_tf(session, worker_id, params):
    """
...
def test_hartmann6_tf_impl():
    ...
    session = tf.Session()
    loss = evaluation_hartmann6_tf(session, 0, params)
    assert np.allclose(loss, -3.32237)

    session.close()


def test_branin_tf_impl():
    # Optimal parameter 1
    x = [-np.pi, 12.275]
    ...
    loss = evaluation_branin_tf(session, 0, params)
    print(loss)
    assert np.allclose(loss, 0.397887)
...
def test_shac_simple():
    ...
    shac = engine.TensorflowSHAC(evaluation_simple_tf, h, total_budget=total_budget, max_gpu_evaluators=0,
    ...
    shac2 = engine.TensorflowSHAC(evaluation_simple_tf, None, total_budget=total_budget, max_gpu_evaluators=0,
...
def test_shac_simple_early_stop():
    ...
    shac = engine.TensorflowSHAC(evaluation_simple_tf, h, total_budget=total_budget, max_gpu_evaluators=1,
                                 num_batches=batch_size, objective=objective)
